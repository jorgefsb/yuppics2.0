...
class my_paypal{
	...
	private $developer_acount_email = ''; // Este es el e-mail que se usa para ingresar a http://developer.paypal.com.


	public $return_url_ok;
	public $return_url_cancel;

	private $PayPalConfig;
	private $PayPal;

	// Array basico de las opciones de la encuenta
	private $SurveyChoices = array('Si', 'No');

	/**
		* Campos basicos y requeridos
		* maxamt
		* returnurl
		* cancelurl
		* brandname
	...
	public function add_products($product){
		...
		foreach ($product as $key => $value) {
			$Item = array(
					'name' => $value['name'], 							// Item name. 127 char max.
					'desc' => '', 							// Item description. 127 char max.
					'amt' => String::float($value['price']), 								// Cost of item.
					'number' => '', 							// Item number.  127 char max.
					'qty' => $value['qty'], 								// Item qty on order.  Any positive integer.
			...
			$total += ($Item['amt'] * $value['qty']);
			array_push($PaymentOrderItems, $Item);
		}

		$this->Payment['amt']         += $total;
		$this->Payment['itemamt']     += $total;

		$this->Payment['order_items'] = $PaymentOrderItems;
	}


	/**
		* Envia la peticion a paypal para procesar la orden y redireccion a paypal
		*/
